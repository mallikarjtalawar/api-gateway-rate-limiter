...
import { developerFetchUsage } from '../api/gateway'
...
export default function DeveloperUsagePage() {
  ...
  const handleFetch = async (e) => {
    ...
    setMetrics(null)
    try {
      const data = await developerFetchUsage(rawKey.trim())
      setMetrics(data)
    } catch (err) {
    ...
    <>
      ...
      <div style={{ padding: '6rem 2rem 2rem', maxWidth: '800px', margin: '0 auto' }}>
        <div className="topbar" style={{ padding: 0, marginBottom: '2rem' }}>
          <div>
            <h1>Usage Metrics</h1>
            <p>Live Redis traffic counters for your API key — this month.</p>
        </div>
      </div>
